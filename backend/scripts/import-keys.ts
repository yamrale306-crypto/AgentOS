...
const BACKEND_ROOT = path.resolve(DIR, '..');
...
function main(): void {
  ...
  const keysPath = fileArg
    ...
    : path.join(BACKEND_ROOT, '..', 'Dev Keys.txt');
  ...
  console.log(`Mode: ${readOnly ? 'READ-ONLY (no files written)' : 'merge into ' + path.relative(BACKEND_ROOT, ENV_PATH)}`);
  ...
  if (!readOnly) {
    writeEnv(merged);
    console.log(`\nWrote ${merged.size} env vars to ${path.relative(BACKEND_ROOT, ENV_PATH)} (${toAdd.length} new).`);
