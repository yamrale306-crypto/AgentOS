...
vi.mock('@agentos/database', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@agentos/database')>();
  return {
    ...actual,
...
import { authenticateBearer, supabaseAdmin, AppError } from '@agentos/database';
...
beforeEach(() => {
  ...
  vi.mocked(authenticateBearer).mockImplementation(async (token: string) => {
    ...
    if (token === 'user-token') return { id: 'user-2', email: 'user@example.test' } as never;
...
describe('health', () => {
  ...
  it('returns public, non-sensitive application version metadata', async () => {
    const res = await request(api).get('/api/app/version');
    expect(res.status).toBe(200);
    expect(res.body.ok).toBe(true);
    expect(res.body.data.version).toMatch(/^\d+\.\d+\.\d+$/);
    expect(res.body.data).not.toHaveProperty('serviceRole');
  });
});
...
describe('task creation', () => {
  it('creates a durable queued task (202) and returns its id', async () => {
...
describe('system', () => {
  ...
  it('rejects authenticated non-admin users', async () => {
    const res = await request(api).get('/api/system/status').set('authorization', 'Bearer user-token');
