...
import { quotaExceeded, tooManyRequests, conflict, badRequest } from '@agentos/database';
import { logger } from '@agentos/config';
import env from '@agentos/config';
import { AI_MODES as APP_AI_MODES } from '@agentos/schemas';

export const tasksRouter: Router = Router();
tasksRouter.use('/api/tasks', requireAuth);
// Authenticated identity avoids proxy/IP false sharing for task reads and mutations.
tasksRouter.use('/api/tasks', rateLimit({
  windowMs: 60 * 1000,
  limit: 90,
  keyGenerator: (req) => req.user!.id,
  standardHeaders: 'draft-8',
  legacyHeaders: false,
  message: { ok: false, error: { code: 'RATE_LIMITED', message: 'Too many task requests. Please try again later.' } }
}));
