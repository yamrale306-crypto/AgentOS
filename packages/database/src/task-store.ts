...
import { ACTIVE_STATUSES, TERMINAL_STATUSES, isActiveStatus, type CreateTaskError, type TaskStatus } from '@agentos/schemas';

export { validatePrompt };

type JsonRecord = { [key: string]: unknown };

const TASK_FIELDS = 'id,user_id,prompt,status,plan,current_step,result,error,steps_used,searches_used,model_used,model_mode,model,provider_used,fallback_used,sources,created_at,updated_at,started_at,completed_at,attempt_count,last_error,lease_owner,lease_expires_at';
...
export async function transition(taskId: string, from: string[], patch: JsonRecord, workerId?: string): Promise<void> {
...
export async function markFailed(taskId: string, message: string, workerId?: string): Promise<void> {
  let query = supabaseAdmin
    .from('tasks')
...
export async function claimNextTask(workerId: string, leaseSeconds: number, maxAttempts: number): Promise<Record<string, unknown> | null> {
  const { data, error } = await supabaseAdmin.rpc('claim_next_task', {
    p_worker_id: workerId, p_lease_seconds: leaseSeconds, p_max_attempts: maxAttempts
  });
  if (error) throw error;
  return data && typeof data === 'object' ? data as Record<string, unknown> : null;
}

export async function heartbeatTask(taskId: string, workerId: string, leaseSeconds: number): Promise<boolean> {
  const { data, error } = await supabaseAdmin.rpc('heartbeat_task', {
    p_task_id: taskId, p_worker_id: workerId, p_lease_seconds: leaseSeconds
  });
  if (error) throw error;
  return data === true;
}