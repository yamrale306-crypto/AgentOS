...
const STATIC_CATALOG: StaticEntry[] = [
  { providerId: 'openai', modelId: 'gpt-4o', label: 'OpenAI GPT-4o', capabilities: { tools: true, vision: true, structuredOutput: true }, contextWindow: 128000, qualityScore: 9, speedScore: 6, costPriority: 3 },
  { providerId: 'openai', modelId: 'gpt-4o-mini', label: 'OpenAI GPT-4o mini', capabilities: { tools: true, structuredOutput: true }, contextWindow: 128000, qualityScore: 8, speedScore: 9, costPriority: 2 },
  { providerId: 'anthropic', modelId: 'claude-3.5-sonnet', label: 'Anthropic Claude 3.5 Sonnet', capabilities: { tools: true, vision: true, structuredOutput: true }, contextWindow: 200000, qualityScore: 9, speedScore: 6, costPriority: 3 },
  { providerId: 'anthropic', modelId: 'claude-3.5-haiku', label: 'Anthropic Claude 3.5 Haiku', capabilities: { tools: true, vision: true, structuredOutput: true }, contextWindow: 200000, qualityScore: 8, speedScore: 9, costPriority: 2 },
  { providerId: 'mistral', modelId: 'mistral-large', label: 'Mistral Large', capabilities: { tools: true, structuredOutput: true }, contextWindow: 128000, qualityScore: 7, speedScore: 7, costPriority: 3 },
...
export const modelCatalog = {
  all: buildStaticModels(),
  types: {} as Record<string, ModelSpec>,
  get size(): number {
    return this.all.length;
  },
  get(id: string): ModelSpec | undefined {
    return this.all.find((m) => m.key === id);
  },
  findClosest(id: string): string[] {
    if (!id) return [];
    const lower = id.toLowerCase();
    const exact = this.all.find((m) => m.key === lower);
    if (exact) return [exact.key];
    const [providerId = '', modelId = ''] = id.split(':');
    let matches = this.all.filter((m) => m.providerId === providerId && m.modelId.toLowerCase().includes(modelId.toLowerCase()));
    if (matches.length === 0) matches = this.all.filter((m) => m.modelId.toLowerCase().includes(modelId.toLowerCase()));
    if (matches.length === 0) matches = this.all.filter((m) => m.providerId === providerId);
    return matches.slice(0, 4).map((m) => m.key);
  }
};