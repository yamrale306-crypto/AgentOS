...
export function buildProviders(input: ProviderEnvInput): ProviderRuntime[] {
  ...
  for (const spec of DEFAULT_PROVIDERS) {
    ...
    providers.push({
      ...
      baseUrl,
      tokenValues: tokens,
      headerTitle: spec.id === 'openrouter' ? 'AgentOS' : null,
      refererUrl: input.APP_URL?.trim() || null
    });
  }
...
export class ProviderManager {
  ...
  clientFor(providerId: string, tokenValue: string): OpenAI {
    ...
    const headers: Record<string, string> = {
      ...
      ...(provider.id === 'openrouter' ? { 'HTTP-Referer': provider.refererUrl ?? 'https://agentos.app' } : {})
    };
