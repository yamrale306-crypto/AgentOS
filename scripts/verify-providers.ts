...
void (async () => {
  ...
  const { default: env } = await import('@agentos/config');
  const { modelRegistry, discoverOpenRouterModels, tokens, providerManager, seedModelRegistry, healthMonitor, normalizeError } = await import('@agentos/ai');
